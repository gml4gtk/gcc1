...
int skip_white_space (void)
{
  ...
  for (;;)
	...
	case '/':
	  c = getc(finput);
	  if (c != '*') {
		/* old fatals("unexpected '/%c' found",c); */
		fprintf (stderr,"unexpected '/%c' found",c);
		exit (1);
	    }
	  c = getc(finput);
	...
	if (c == '\\')
	  {
		...
		if (code >= 128 || code<0) { /* JF this said if(c>=128) */
		    ...
		    fprintf(stderr,"malformatted literal token '\\%03o'",code);
		    exit (1);
		  }
	      }
	    else
		...
		if (c == 't')
		  ...
		  code = '\"';
			...
			fprintf(stderr,"invalid literal token '\\%c'",c);
			exit (1);
		    }
		c = getc(finput);
	      }
	...
	*p = 0;
      ...
      while (c != '>')
	{
	  ...
	  if (p >= token_buffer + MAXTOKEN - 1) {
		...
		fprintf(stderr,"type name too long (%d max)",MAXTOKEN-1);
		exit (1);
	    }
