      ...
      if (c == '%')
	{
	  ...
	  switch (tok)
        ...
        fatal("no input grammar");
    	    ...
    	    fprintf(stderr,"Unrecognized char '%c' in declaration section",c);
    	    exit (1);
	}
    }
}
...
/*      if (lineno != start_lineno)
      ...
      else if (token == IDENTIFIER)
	{
	  if (symval->class == what_is_not) {
		...
		fprintf(stderr,"symbol %s redefined", symval->tag);
		exit (1);
	    }
	  ...
	  if (typename)
		...
		symval->type_name = typename;
		    ...
		    fprintf(stderr,"type redeclaration for %s", symval->tag);
		    exit (1);
		}
	    }
	}
  ...
  while (t != TWO_PERCENTS && t != ENDFILE)
      ...
      if (t == IDENTIFIER || t == BAR)
	{
	  ...
	  if (lhs->class == SUNKNOWN)
		...
		fprintf(stderr,"rule given for %s, which is a token", lhs->tag);
		exit (1);
	    }
...
static void packsymbols (void)
{
  ...
  for (bp = firstsymbol; bp; bp = bp->next)
      ...
      if (translations)
	{
	  if (token_translations[bp->user_token_number] != 2)
		...
		*/
		fprintf(stderr,"tokens %s and %s both assigned number %d",
		tags[token_translations[bp->user_token_number]],
		bp->tag,
		bp->user_token_number);
		exit (1);
  ...
  if (startval->class == SUNKNOWN) {
	...
	fprintf(stderr,"the start symbol %s is undefined", startval->tag);
	exit (1);
  }
  else if (startval->class == STOKEN) {
	/* fatals("the start symbol %s is a token", startval->tag); */
	fprintf(stderr,"the start symbol %s is a token", startval->tag);
	exit (1);
  }
