...
void gccerror (char *msg, char *arg1, char *arg2);
 ...
 **/
void
record_temp_file (char *filename, int success_only)
{
 ...
 **/
void
store_arg (char *arg, int tempnamep)
{
  if (argbuf_index + 1 == argbuf_length)
    {
      argbuf =
	(char **) realloc (argbuf, (argbuf_length *= 2) * sizeof (char *));
    }
...
execute (void)
{
  ...
  if ((status & 0x7F) != 0)
    {
      fatal ("Program %s got fatal signal %d.", argbuf[0],
	     (char *) (status & 0x7F));
    }
 ...
 **/
char *
make_switch (char *p1, int s1, char *p2, int s2)
{
  register char *new;
  if (p2 && s2 == 0)
    {
...
int
do_spec_1 (char *spec, int inswitch)
{
	...
	case '%':
	  switch (c = *p++)
	    ...
	    case 'b':
	      ...
	      arg_going = 1;
	      break;
	      /* %p  substitutes the standard macro predefinitions */
	    case 'p':
	      do_spec_1 (CPP_PREDEFINES, 0);
	      break;
	      /* %g     substitute the temporary-file-name-base */
	    case 'g':
	      obstack_grow (&obstack, temp_filename, temp_filename_length);
	      delete_this_arg = 1;
	      arg_going = 1;
	      break;
	      /* %d  marks the argument following the %d as a temporary file name */
	    case 'd':
	      delete_this_arg = 2;
	      break;
	      /* %w  marks the argument following the %w as output file */
	    case 'w':
	    ...
	    case 's':
	      ...
	      break;
	      /* %o  substitutes the names of all the output files */
	    case 'o':
	      {
		register int f;
	    ...
	    case 'L':
	      ...
	      break;
	      /* %S     process STARTFILE_SPEC as a spec */
	    case 'S':
	      do_spec_1 (STARTFILE_SPEC, 0);
	      break;
	      /*  %{S*}  substitutes all the switches */
	    case '{':
	      p = handle_braces (p);
	      if (p == 0)
		{
 ...
 **/
void
fatal (char *msg, char *arg1, char *arg2)
{
  gccerror (msg, arg1, arg2);
  delete_temp_files (0);
  exit (1);
}
 ...
 **/
void
gccerror (char *msg, char *arg1, char *arg2)
{
