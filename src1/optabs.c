     ...
     enum optab_methods methods;
{
  ...
  if (binoptab == add_optab
      ...
      || binoptab == smul_optab
      || binoptab == umul_optab
      || binoptab == smul_widen_optab || binoptab == umul_widen_optab)
    {
  ...
  if (binoptab->handlers[(int) mode].insn_code != CODE_FOR_nothing)
      ...
      if (GET_MODE (op0) != VOIDmode && GET_MODE (op0) != mode0)
	{
      ...
      if (GET_MODE (xop1) != VOIDmode && GET_MODE (xop1) != mode1)
	{
...
int
expand_twoval_binop_convert (binoptab, mode, op0, op1, targ0, targ1,
			     unsignedp)
     ...
     enum rtx_code code;
{
  ...
  if (code != UNKNOWN && insn != NEXT_INSN (prev_insn))
    REG_NOTES (insn)
      = gen_rtx (EXPR_LIST, REG_EQUAL,
		 gen_rtx (code, GET_MODE (temp), op0), 0);
     ...
     int unsignedp;
{
  ...
  if (fun =
      can_fix_p (GET_MODE (to), GET_MODE (from), unsignedp, &must_trunc))
    {
      if (must_trunc)
	from = ftruncify (from);
      emit_insn ((*fun) (to, from));
