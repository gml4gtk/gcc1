	  ...
	  if (INSN_UID (first_insn) > INSN_UID (f->before_jump)
	      && !TREE_ADDRESSABLE (f->target))
	    {
	      error_with_file_and_line (DECL_SOURCE_FILE (f->target),
					...
					"label \"%s\" was used \
before containing binding contour", IDENTIFIER_POINTER (DECL_NAME (f->target)));
	  ...
	  if (f->stack_level)
	    emit_insn_after (gen_move_insn
			     (stack_pointer_rtx, f->stack_level),
			     f->before_jump);
	  f->before_jump = 0;
	}
...
expand_asm (body)
     tree body;
{
  emit_insn (gen_rtx (ASM_INPUT, VOIDmode, TREE_STRING_POINTER (body)));
  last_expr_type = 0;
}
  ...
  for (i = 0, tail = outputs; tail; tail = TREE_CHAIN (tail), i++)
    {
	  ...
	  && TREE_CODE (val) != PARM_DECL && TREE_CODE (val) != INDIRECT_REF)
  ...
  if (noutputs == 1)
    {
      ...
      emit_insn (gen_rtx (SET, VOIDmode,
			  expand_expr (val, 0, VOIDmode, 0), body));
    }
  else
    {
  ...
  if (optimize
	...
	&& GET_CODE (SET_SRC (PATTERN (insn))) == IF_THEN_ELSE))
    {
      /* Scan insns from the top of the loop looking for a qualified
         conditional exit.  */
      for (insn = loop_stack->data.loop.start_label; insn;
	   insn = NEXT_INSN (insn))
     ...
     tree retval;
{
  ...
  if (optimize && really_for_value
      ...
      && TREE_OPERAND (TREE_OPERAND (TREE_OPERAND (retval, 1), 0),
		       0) == this_function
	   ...
	   chain = chain->next)
	{
	  ...
	  if (TREE_ADDRESSABLE (chain->label))
	    error_with_file_and_line (DECL_SOURCE_FILE (chain->label),
				      ...
				      "label \"%s\" was used \
before containing binding contour", IDENTIFIER_POINTER (DECL_NAME (chain->label)));
	}
  ...
  if (obey_regdecls)
	...
	if (TREE_CODE (decl) == VAR_DECL
	    && DECL_RTL (decl) != 0 && GET_CODE (DECL_RTL (decl)) == REG)
...
expand_end_case ()
{
  ...
  if (index_expr != error_mark_node)
      ...
      else
	{
	  ...
	  emit_jump_insn (gen_casesi
			  (index, expand_expr (minval, 0, VOIDmode, 0),
			   expand_expr (range, 0, VOIDmode, 0), table_label,
			   default_label));
#else
	  ...
	  do_tablejump (index,
			gen_rtx (CONST_INT, VOIDmode,
				 TREE_INT_CST_LOW (range)), table_label,
			default_label);
#else
	  lossage;
...
#endif
  return change_address (SUBREG_REG (x), mode, plus_constant (addr, offset));
}

#if 0
     ...
     rtx *equiv_mem;
{
  ...
  if (GET_CODE (XEXP (bitfield, 1)) == CONST_INT
	...
	memref = SUBREG_REG (XEXP (bitfield, 0));
	       ...
	       && (memref =
		   equiv_mem[REGNO (SUBREG_REG (XEXP (bitfield, 0)))]) != 0)
	;
	  ...
	  && offsetable_address_p (GET_MODE (bitfield), XEXP (memref, 0)))
	{
	  ...
	  memref = gen_rtx (MEM,
			    (INTVAL (XEXP (bitfield, 1)) ==
			     GET_MODE_BITSIZE (QImode) ? QImode : HImode),
			    XEXP (memref, 0));
	  ...
	  if (destflag)
	    {
	      SET_DEST (body) = adj_offsetable_operand (memref, offset);
	  ...
	  else
	    {
	      ...
	      emit_insn_before (gen_extend_insn
				(newreg,
  ...
  for (parm = DECL_ARGUMENTS (fndecl); parm; parm = TREE_CHAIN (parm))
	...
	if (nregs > 0)
	  current_function_pretend_args_size
	    =
	    (((nregs * UNITS_PER_WORD) + (PARM_BOUNDARY / BITS_PER_UNIT) -
	      1) / (PARM_BOUNDARY / BITS_PER_UNIT) * (PARM_BOUNDARY /
						      BITS_PER_UNIT));
      ...
      if (nominal_mode != BLKmode && nominal_mode != passed_mode)
	{
	  ...
	  stack_offset.constant
	    += GET_MODE_SIZE (passed_mode) - GET_MODE_SIZE (nominal_mode);
	  stack_offset_rtx = ARGS_SIZE_RTX (stack_offset);
#endif
		     ...
		     && TREE_CODE (TREE_TYPE (parm)) == REAL_TYPE)))
	{
	  ...
	  if (nominal_mode == passed_mode
	      && GET_CODE (entry_parm) == MEM && stack_offset.var == 0)
	    {
      ...
      if (TREE_READONLY (parm))
	{
      ...
      FUNCTION_ARG_ADVANCE (args_so_far, passed_mode, DECL_ARG_TYPE (parm),
			    1);
    }
