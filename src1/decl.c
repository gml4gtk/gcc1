...
poplevel (int keep, int reverse, int functionbody)
{
  ...
  if (keep || functionbody)
    {
      block =
	build_let (0, 0, keep ? decls : 0, subblocks, 0, keep ? tags : 0);
    }
...
static int
duplicate_decls (tree new, tree old)
{
...
implicitly_declare (tree functionid)
{
  ...
  if (current_binding_level == global_binding_level || flag_traditional)
    {
...
static char *
redeclaration_error_message (tree new, tree old)
{
...
init_decl_processing (void)
{
  ...
  char_type_node =
    (flag_signed_char
     ? make_signed_type (BITS_PER_UNIT) : make_unsigned_type (BITS_PER_UNIT));
  ...
  short_integer_type_node =
    make_signed_type (BITS_PER_UNIT * min (UNITS_PER_WORD / 2, 2));
  pushdecl (build_decl
	    (TYPE_DECL, get_identifier ("short int"),
	     short_integer_type_node));
  ...
  short_unsigned_type_node =
    make_unsigned_type (BITS_PER_UNIT * min (UNITS_PER_WORD / 2, 2));
  pushdecl (build_decl
	    (TYPE_DECL, get_identifier ("short unsigned int"),
	     short_unsigned_type_node));
...
/*  builtin_function ("__builtin_div", default_ftype, BUILT_IN_DIV);
  ...
  builtin_function ("__builtin_frem", double_ftype_double_double,
		    BUILT_IN_FREM);
  builtin_function ("__builtin_memcpy", void_ftype_ptr_ptr_int,
		    BUILT_IN_MEMCPY);
  builtin_function ("__builtin_memcmp", int_ftype_ptr_ptr_int,
		    BUILT_IN_MEMCMP);
  builtin_function ("__builtin_memset", void_ftype_ptr_int_int,
		    BUILT_IN_MEMSET);
...
groktypename (tree typename)
{
  ...
  return grokdeclarator (TREE_VALUE (typename),
			 TREE_PURPOSE (typename), TYPENAME, 0);
}
...
void
finish_decl (tree decl, tree init, tree asmspec)
{
		...
		enum decl_context decl_context, int initialized)
{
...

  {
	...
	if (specbits & (1 << (int) RID_REGISTER))
	  {
	    error ("top-level declaration of `%s' specifies `register'",
		   name);
	  }
      }
  }
  ...
  while (declarator && TREE_CODE (declarator) != IDENTIFIER_NODE)
      ...
      if (TREE_CODE (declarator) == ARRAY_REF)
	{
	  ...
	  if (size)
		...
		itype =
		  make_index_type (build_int_2
				   (TREE_INT_CST_LOW (size) - 1, 0));
	      else
		{
		  if (pedantic)
		    {
		      warning ("ANSI C forbids variable-size array `%s'",
			       name);
		    }
		  itype =
		    build_binary_op (MINUS_EXPR, size, integer_one_node);
      ...
      else if (TREE_CODE (declarator) == INDIRECT_REF)
	{
	  ...
	  if (TREE_TYPE (declarator))
		...
		{
		  if (TREE_VALUE (typemodlist) ==
		      ridpointers[(int) RID_CONST])
		    {
		      constp++;
		    }
...
static tree
grokparms (tree first_parm, int funcdef_flag)
{
  if (first_parm != 0
...
tree
get_parm_types (int void_at_end)
{
  register tree decl;
...
tree
grokfield (char *filename, int line, tree declarator, tree declspecs,
	   tree width)
{
...
tree
finish_struct (tree t, tree fieldlist)
{
  register tree x;
  int old;
  int round_up_size = 1;
  ...
  if (current_binding_level->n_incomplete != 0)
    {
      tree decl;
      for (decl = current_binding_level->names; decl;
      ...
      for (parm = parmdecls; parm;)
	{
	  ...
	  if (TREE_CODE (parm) != PARM_DECL)
	  ...
	  else if (DECL_CONTEXT (parm) == 0)
	    {
	      ...
	      specparms
		= chainon (specparms, tree_cons (parm, NULL_TREE, NULL_TREE));
	    }
...
void
finish_function (char *filename, int line)
{
  register tree fndecl = current_function_decl;
