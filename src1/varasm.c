 ...
 **/
void
assemble_variable (tree decl, tree asmspec, int top_level, int write_symbols,
		   int at_end)
{
  ...
  if (DECL_RTL (decl) == 0 || GET_MODE (DECL_RTL (decl)) != DECL_MODE (decl))
    {
  ...
  if (DECL_INITIAL (decl) == 0)
    {
      ...
      if (TREE_PUBLIC (decl))
	{
	  ASM_OUTPUT_COMMON (asm_out_file, name, size, 1 /* rounded */ );
	}
      else
	{
	  ASM_OUTPUT_LOCAL (asm_out_file, name, size, 1 /* rounded */ );
	}
      return;
    }
...

{
  ...
  for (r = real_constant_chain; r; r = XEXP (r, 3))
    if (XINT (r, 0) == i0 && XINT (r, 1) == i1 && GET_MODE (r) == mode)
      return r;
     ...
     struct addr_const *value;
{
  ...
  if (TREE_CODE (target) == VAR_DECL || TREE_CODE (target) == FUNCTION_DECL)
    {
      x = DECL_RTL (target);
    }
     ...
     tree exp;
{
  ...
  TREE_CST_RTL (exp) = gen_rtx (MEM, TYPE_MODE (TREE_TYPE (exp)), def);
     ...
     int size;
{
  ...
  for (link = CONSTRUCTOR_ELTS (exp);
    ...
    {
      if (field == 0 || (DECL_MODE (field) != BImode))
	{
	  ...
	  if (field
	      && (total_bytes * BITS_PER_UNIT) % DECL_ALIGN (field) != 0)
	    {
