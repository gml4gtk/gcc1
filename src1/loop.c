     ...
     int nregs;
{
  ...
  while (p != end && GET_CODE (p) != CODE_LABEL && GET_CODE (p) != JUMP_INSN)
    p = NEXT_INSN (p);
  ...
  count_loop_regs_set (loop_top ? loop_top : loop_start, end,
		       n_times_set, may_not_move, &insn_count, nregs);
  ...
  while (1)
	  ...
	  && !may_not_move[REGNO (SET_DEST (PATTERN (p)))])
	{
	  ...
	  if ((uid_luid[regno_last_uid[REGNO (SET_DEST (PATTERN (p)))]] >
	       INSN_LUID (end)
	       || uid_luid[regno_first_uid[REGNO (SET_DEST (PATTERN (p)))]] <
	       INSN_LUID (loop_start)) && (maybe_never
					   || loop_reg_used_before_p (p,
								      loop_start,
								      scan_start,
								      end)))
	    ;
...

  {
	...
	for (m = m1->next; m; m = m->next)
	  if (INSN_UID (m->insn) == regno_last_uid[regno])
	    break;
	if (m != 0
	    && SET_SRC (PATTERN (m->insn)) == SET_DEST (PATTERN (m1->insn)))
	  m = 0;
...

  {
      ...
      if (m->match == 0 && n_times_used[m->regno] == 1)
	{
	  ...
	  for (m1 = m->next; m1; m1 = m1->next)
		...
		&& !m1->global &&(matched_regs[m1->regno] || (
	  ...
	  && (!m->forces || m->forces->done))
	{
	  ...
	  if (already_moved[regno]
	      ...
	      for (count = m->consec; count >= 0; count--)
		{
		  ...
		  if (REG_NOTES (i1) == 0 && !m->partial	/* But not if its a zero-extend clr. */
     ...
     rtx p, loop_start;
{
  ...
  new = gen_rtx (SET, VOIDmode,
		 gen_rtx (STRICT_LOW_PART, VOIDmode,
			  gen_rtx (SUBREG,
				   GET_MODE (XEXP (SET_SRC (PATTERN (p)), 0)),
     ...
     short *n_times_set;
{
  ...
  switch (code)
    ...
    case REG:
      if (x == frame_pointer_rtx || x == arg_pointer_rtx || x->unchanging)
	return 1;
	...
	return 0;
      ...
      if (fixed_aggregate_altered && rtx_addr_varies_p (x))
	return 0;
     ...
     int size;
{
  ...
  if (GET_CODE (base) == CONST)
    base = XEXP (base, 0);
  if (GET_CODE (base) == PLUS && GET_CODE (XEXP (base, 1)) == CONST_INT)
    {
     ...
     rtx x;
{
  switch (GET_CODE (x))
    ...
    case UDIV:
    case UMOD:
      if (!CONSTANT_P (XEXP (x, 1)) && GET_CODE (XEXP (x, 1)) != CONST_DOUBLE)
	return 1;
    }
  return 0;
}
