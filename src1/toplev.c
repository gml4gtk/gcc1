...
//    fprintf (asm_out_file, ASM_FILE_START);
  ...
  if (write_symbols == 2)
    TIMEVAR (symout_time,
	     {
	     dbxout_tags (gettags ()); dbxout_types (get_permanent_types ());
	     }
  );
  /* Do gdb symbols */
  if (write_symbols == 1)
...
void
rest_of_decl_compilation (tree decl, tree asmspec, int top_level, int at_end)
{
     ...
     tree decl;
{
  ...
  if (DECL_SAVED_INSNS (decl) == 0)
      ...
      if (flag_inline_functions || TREE_INLINE (decl))
	{
	  TIMEVAR (integration_time,
		   ...
		   if (lose == 0) save_for_inline (decl);
		   else
		   TREE_INLINE (decl) = 0;}
	  );
	}
	...
	TIMEVAR (dump_time,
		 ...
		 print_rtl (rtl_dump_file, insns); fflush (rtl_dump_file);}
      );
	  ...
	  && TREE_INLINE (decl) && !flag_keep_inline_functions)
	{
  ...
  if (cse_dump)
    {
      TIMEVAR (dump_time,
	       ...
	       fprintf (cse_dump_file,
			...
			IDENTIFIER_POINTER (DECL_NAME
					    (decl)));
	       print_rtl (cse_dump_file, insns); fflush (cse_dump_file);
	       }
      );
    }
  if (loop_dump)
  ...
  if (loop_dump)
    TIMEVAR (dump_time,
	     {
	     print_rtl (loop_dump_file, insns); fflush (loop_dump_file);
	     }
  );
  ...
  if (flow_dump)
    TIMEVAR (dump_time,
	     {
	     print_rtl (flow_dump_file, insns); fflush (flow_dump_file);
	     }
  );
  ...
  if (combine_dump)
    TIMEVAR (dump_time,
	     ...
	     print_rtl (combine_dump_file, insns); fflush (combine_dump_file);
	     }
  );
  ...
  if (!obey_regdecls)
    TIMEVAR (local_alloc_time,
	     {
	     regclass (insns, max_reg_num ()); local_alloc ();
	     }
  );
  ...
  for (i = 1; i < argc; i++)
      ...
      if (argv[i][0] == '-')
	{
	  ...
	  if (str[0] == 'm')
		...
		{
		  cc1error ("Invalid option, `%s'", argv[i]);
		}
	    }
...
#define __VERSION__ "[unknown]"
	    ...
	    {
	      cc1error ("Invalid switch, %s.", argv[i]);
	    }
	}
      else
	{
