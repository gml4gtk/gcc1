     ...
     rtx x;
{
  ...
  if (code == REG)
    return !(REGNO (x) == FRAME_POINTER_REGNUM
	     || REGNO (x) == ARG_POINTER_REGNUM || x->unchanging);
     ...
     int regno;
{
  ...
  for (link = REG_NOTES (insn); link; link = XEXP (link, 1))
    if (REG_NOTE_KIND (link) == kind && REGNO (XEXP (link, 0)) == regno)
      return link;
  return 0;
}

      ...
      case 's':
	{
	  ...
	  while (1)
	      ...
	      if (j >= stringbufsize - 4)
		{
		  stringbufsize *= 2;
		  stringbuf =
		    (char *) xrealloc (stringbuf, stringbufsize + 1);
		}
