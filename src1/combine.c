 ...
 **/
void
combine_instructions (rtx f, int nregs)
{
  register rtx insn;
  register int i;
	  ...
	  if (GET_CODE (insn) == INSN
		...
		if (GET_CODE (XEXP (links, 0)) != NOTE)
		  for (nextlinks = XEXP (links, 1); nextlinks;
		       nextlinks = XEXP (nextlinks, 1))
		    if (try_combine
			(insn, XEXP (links, 0), XEXP (nextlinks, 0)))
		      goto retry;
	    }
#endif
     ...
     register rtx i3, i2, i1;
{
  ...
  if (GET_CODE (i2) == NOTE || (i1 && GET_CODE (i1) == NOTE))
    return 0;
  ...
  added_sets_2 = (GET_CODE (i2dest) != CC0 && !dead_or_set_p (i3, i2dest));
  if (i1)
  ...
  if ((i2_is_used > 1 && find_reg_note (i2, REG_INC, 0) != 0)
      || (i1 != 0 && n_occurrences > 1
	  && find_reg_note (i1, REG_INC, 0) != 0))
    {
      undo_all ();
      return 0;
    }
  ...
  if (insn_code_number >= 0)
      ...
      if (GET_CODE (i2dest) == REG)
	{
	  ...
	  if (!added_sets_2 && !reg_mentioned_p (i2dest, PATTERN (i3)))
	  ...
	  if (GET_CODE (i1dest) == REG)
	      ...
	      if (!added_sets_1)
		{
	      ...
	      /* If a ref to REGNO was substituted into I3 from I1,
	         ...
	         or this entire life of REGNO is gone so remove its death.  */
	      if (!added_sets_1 && !reg_mentioned_p (i1dest, PATTERN (i3)))
		remove_death (regno, i3);
	    }
...
#define FAKE_EXTEND_SAFE_P(MODE, FROM) (GET_CODE (FROM) == REG)
  ...
  switch (code)
    ...
    case NEG:
      ...
      if (was_replaced[0] && GET_CODE (to) == code)
	return XEXP (to, 0);
      break;
	...
	{
	  int sum = (INTVAL (XEXP (x, 1)) + INTVAL (XEXP (XEXP (x, 0), 1)));
	  if (sum == 0)
	  ...
	  || ((was_replaced[0] || was_replaced[1]) && GET_CODE (to) == PLUS))
	{
    ...
    case NE:
      ...
      if (was_replaced[0]
	  && GET_CODE (to) == SUBREG
	  && SUBREG_WORD (to) == 0 && XEXP (x, 1) == const0_rtx)
	SUBST (XEXP (x, 0), SUBREG_REG (to));
      ...
      if (was_replaced[0]
	  && GET_CODE (to) == ZERO_EXTRACT && XEXP (x, 1) == const0_rtx)
	{
	  if (!undobuf.storage)
	    undobuf.storage = (char *) oballoc (0);
	  SUBST (XEXP (x, 0),
		 gen_rtx (SIGN_EXTRACT, GET_MODE (to),
			  XEXP (to, 0), XEXP (to, 1), XEXP (to, 2)));
	}
      /* If we are putting (ASHIFT 1 x) into (EQ (AND ... y) 0),
         ...
         which is what jump-on-bit instructions are written with.  */
	       ...
	       && (XEXP (XEXP (x, 0), 0) == to
		   || XEXP (XEXP (x, 0), 1) == to)
	       && GET_CODE (to) == ASHIFT && XEXP (to, 0) == const1_rtx)
	{
	  ...
	  SUBST (XEXP (x, 0),
		 gen_rtx (SIGN_EXTRACT, GET_MODE (to),
			  y, const1_rtx, XEXP (to, 1)));
	}

      break;

    case ZERO_EXTEND:
      if (was_replaced[0] && GET_CODE (to) == ZERO_EXTEND)
	SUBST (XEXP (x, 0), XEXP (to, 0));
	  ...
	  && 0 == (INTVAL (XEXP (to, 1)) & ~GET_MODE_MASK (GET_MODE (to))))
	{
	  if (!undobuf.storage)
	    undobuf.storage = (char *) oballoc (0);
    ...
    case SIGN_EXTEND:
      if (was_replaced[0] && GET_CODE (to) == SIGN_EXTEND)
	SUBST (XEXP (x, 0), XEXP (to, 0));
	  ...
	  && FAKE_EXTEND_SAFE_P (GET_MODE (x), XEXP (to, 0))
	  && ((INTVAL (XEXP (to, 1))
	       & (-1 << (GET_MODE_BITSIZE (GET_MODE (to)) - 1))) == 0))
	{
	  if (!undobuf.storage)
	    undobuf.storage = (char *) oballoc (0);
	  return gen_rtx (AND, GET_MODE (x),
	...
	{
	  SUBST (SUBREG_REG (XEXP (x, 1)),
		 XEXP (SUBREG_REG (XEXP (x, 1)), 0));
	}
...
#else
	  int shiftcount = INTVAL (XEXP (XEXP (x, 0), 2));
#endif
	  if (!undobuf.storage)
	    undobuf.storage = (char *) oballoc (0);
	  return
    ...
    case ZERO_EXTRACT:
      ...
      if (was_replaced[0] && GET_CODE (to) == TRUNCATE)
	{
#ifdef BITS_BIG_ENDIAN
	  ...
	  && GET_CODE (XEXP (to, 1)) == CONST_INT
	  && XEXP (x, 1) == const1_rtx && GET_CODE (XEXP (x, 2)) == CONST_INT)
	{
	  ...
	  if (newpos >= 0 && newpos < GET_MODE_BITSIZE (GET_MODE (to)))
	    {
	      if (!undobuf.storage)
		undobuf.storage = (char *) oballoc (0);
	      return gen_rtx (ZERO_EXTRACT, GET_MODE (x),
	  ...
	  && (GET_CODE (to) == SIGN_EXTEND || GET_CODE (to) == ZERO_EXTEND))
	{
	  if (!undobuf.storage)
	    undobuf.storage = (char *) oballoc (0);
	  SUBST (XEXP (x, 1),
		 gen_rtx (SUBREG, GET_MODE (to), XEXP (to, 0), 0));
	}
#endif
     ...
     rtx x, to;
{
  ...
  if (GET_CODE (varop) == SUBREG
      ...
      && GET_CODE (to) == AND && GET_CODE (XEXP (to, 1)) == CONST_INT
  ...
  if (varop == to
	...
	undobuf.storage = (char *) oballoc (0);
      return gen_rtx (AND, GET_MODE (x),
		      ...
		      gen_rtx (CONST_INT, VOIDmode,
			       constop &
			       GET_MODE_MASK (GET_MODE (XEXP (to, 0)))));
    }
  ...
  if (GET_CODE (varop) == ASHIFTRT
      ...
      if (constop == GET_MODE_MASK (QImode))
	{
      ...
      return gen_rtx (AND, GET_MODE (x), XEXP (to, 0), XEXP (x, 1));
    }
...
#endif
      return gen_rtx (MEM, mode, plus_constant (XEXP (x, 0), offset));
    }
  else
    return gen_rtx (CLOBBER, VOIDmode, const0_rtx);
}
     ...
     rtx insn;
{
  ...
  newpat = gen_rtx (SET, VOIDmode, cc0_rtx,
		    gen_rtx (ZERO_EXTRACT, VOIDmode, var, const1_rtx,
			     bitnum));
     ...
     rtx insn;
{
  ...
  if (GET_CODE (PATTERN (insn)) == PARALLEL)
	...
	reg_last_set[REGNO (x)] = insn;
      if (GET_CODE (x) == SUBREG && GET_CODE (SUBREG_REG (x)) == REG)
